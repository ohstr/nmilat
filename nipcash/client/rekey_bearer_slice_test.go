...
func TestRekeyBearerSlice_NoConsolidateWith_OneTransferOnly(t *testing.T) {
	...
	result, err := rekeyBearerSlice(context.Background(), fake, RekeyBearerSliceParams{
		BearerSlice: nipcash.Source{WalletPubkey: "wallet1", Amount: 5000, Credential: nipcash.BySecret("old-secret")},
	})
	if err != nil {
		t.Fatalf("rekeyBearerSlice: %v", err)
	}
...
func TestRekeyBearerSlice_ConsolidateWith_TransfersThenConsolidates(t *testing.T) {
...
func TestRekeyBearerSlice_BadInterimIdentity_RejectedBeforeAnyWireCall(t *testing.T) {
	// The exact fund-safety issue independent review caught: a bearer
	...
	_, err := rekeyBearerSlice(context.Background(), fake, RekeyBearerSliceParams{
		BearerSlice:     nipcash.Source{WalletPubkey: "wallet1", Amount: 5000, Credential: nipcash.BySecret("old-secret")},
		InterimIdentity: nipcash.NewBearerTarget(), // invalid: not a pubkey target
...
func TestRekeyBearerSlice_InterimTransferLandsButConsolidateFails_PartialProgress(t *testing.T) {
	const privKeyHex = "a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1a1"
	...
	if partial.Consolidated != nil {
		t.Fatal("partial.Consolidated must be nil — RekeyBearerSlice's interim step is a transfer, not a consolidate")
	}
