...
func TestCashRedeemParams_Request_Cash(t *testing.T) {
	p := CashRedeemParams{Invoice: testInvoice, Credential: BySecret("mysecret")}
	req, err := p.Request(randomKeyHex(t))
	if err != nil {
		t.Fatalf("Request: %v", err)
	}
	if req.CashSecret != "mysecret" {
		t.Fatalf("CashSecret: got %q", req.CashSecret)
	}
	if req.IdentityType != "" || req.IdentityEvent != "" {
		t.Fatalf("a cash-mode request must carry no identity fields: %+v", req)
	}
}
...
func TestCashRedeemParams_Request_Signing(t *testing.T) {
	...
	if req.CashSecret != "" {
		t.Fatalf("a signing request must carry no cash_secret: %+v", req)
	}
}
