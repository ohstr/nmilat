...
func TestSupportedNIPsCore(t *testing.T) {
	...
	for _, conditional := range []int{42, 26, 50} {
		if hasNIP(got, conditional) {
			t.Errorf("SupportedNIPs() = %v, did not expect conditional NIP %d with no auth/delegation/search configured", got, conditional)
		}
	}
}
